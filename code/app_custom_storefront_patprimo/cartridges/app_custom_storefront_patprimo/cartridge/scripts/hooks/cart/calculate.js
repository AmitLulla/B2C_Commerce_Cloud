...
exports.calculate = function (basket) {
    ...
    HookMgr.callHook('app.update.giftcertificate', 'updategiftCard', basket);
...
exports.calculateShipping = function (basket) {
    ...
    if (session.custom.selectedshipping && session.custom.shippingCostDepartamento) {
        costo = session.custom.shippingCostDepartamento;
        costo = Number(costo, basket.currencyCode);
        Transaction.wrap(function () {
            collections.forEach(shipments, function (shipment) {
                var lineItems = shipment.getShippingLineItems();
                collections.forEach(lineItems, function (lineItem) {
                    lineItem.setPriceValue(costo);
                });
            });
            // basket.updateTotals();
        });
