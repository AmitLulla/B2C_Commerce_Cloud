...
server.replace('Find', server.middleware.https, consentTracking.consent, function (req, res, next) {
    ...
    if (!radius)radius = '40000';
    session.custom.countryCode = req.querystring.country ? req.querystring.country: null;
    ...
    var showMap = true;
    var radiusExtended = true;

    var extendRadiusWhenSearchEmpty = req.querystring.extendRadiusWhenSearchEmpty !== 'false';
    var stores = storeHelpers.getStores(radius, postalCode, lat, long, req.geolocation, showMap, city, storeID);
    var stores2 = storeHelpers.getStores('40000', '', '', '', req.geolocation, showMap);
    var allStoreHtml = storeHelpers.createStoresResultsHtml(stores2.stores,true,postalCode);
    ...
    if (extendRadiusWhenSearchEmpty && stores.stores.length === 0 && !city) {
        stores = storeHelpers.getStores(-1, postalCode, lat, long, req.geolocation, showMap);
    }
    var StoreMgr = require('dw/catalog/StoreMgr');
    var query = 'custom.enable = true';
    var StoreQuery = StoreMgr.searchStoresByCoordinates(0, 0, 'km', 40000, query);
    var allStores = StoreQuery.keySet();

    var countrysArray = [];
    for (var index in allStores) {
        if (countrysArray.indexOf(allStores[index].countryCode.displayValue + '-' + allStores[index].countryCode.value) === -1) {
            countrysArray.push(allStores[index].countryCode.displayValue + '-' + allStores[index].countryCode.value);
        }
    ...
    if (update) {
        ...
        stores.storesResultsHtml2 = allStoreHtml;
        stores.storesResultsHtml;
        stores.resultados= Resource.msgf('results', 'shoppingthestore', null,stores.stores.length);
        res.json(stores);
    } else {
        var viewData = {};
        if (preferredStoreId) {
        ...
        viewData.country = countrysArray;
        viewData.stores = stores2;
        viewData.showMap = showMap;
        viewData.radiusExtended = radiusExtended;
        viewData.stores.resultados= Resource.msgf('results', 'shoppingthestore', null,viewData.stores.stores.length);
...
/**
 * Get Citys from Stores
 */
server.get('GetCity', function (req, res, next) {
    var StoreMgr = require('dw/catalog/StoreMgr');
    var country = req.httpParameterMap.country.value;
    var query = 'countryCode = {0}';
    var arrayCitys = [];
    try {
        var storeMgrResult = StoreMgr.searchStoresByCoordinates(0, 0, 'km', 40000, query, country);
    } catch (error) {
    }
    var stores = storeMgrResult.keySet();
    if (stores.length > 0) {
            for (var key in stores) {
                if (stores[key].custom.enable === true) {
                    if (arrayCitys.indexOf(stores[key].city.toUpperCase()) === -1) {
                        arrayCitys.push(stores[key].city.toUpperCase());
                    }
                }
            }
    }

    res.json({citys:arrayCitys ? arrayCitys : []})
    next();
});
