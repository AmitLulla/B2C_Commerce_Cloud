...
var MAX_NB_STORE_RESULTS = dw.system.Site.getCurrent().getCustomPreferenceValue('show_limit') || 10;
 ...
 * @returns {string} The rendered HTML
 */
function createStoresResultsHtml(storesInfo, showMap,postalCode) {
    var Template = require('dw/util/Template');
   
    var context = new HashMap();
    if (postalCode && storesInfo) {
        for (var key in storesInfo) {
            if (storesInfo[key].postalCode === postalCode) {
                storesInfo[key].selected = true;
            }
        }
    }
 ...
 */
function getStores(radius, postalCode, lat, long, geolocation, showMap, url, storeId) {
    ...
    var city = url ? url : null;
    var apiKey = Site.getCurrent().getCustomPreferenceValue('api_google');
    ...
    if (storeId && storeId !== '' ) {
        var queryString = 'ID = {0} and custom.enable = true';
        storeMgrResult = StoreMgr.searchStoresByCoordinates(location.lat, location.long, distanceUnit, resolvedRadius, queryString, storeId);
        searchKey = { postalCode: postalCode };
    } else if (postalCode && postalCode !== '') {
        var queryString = 'name ILIKE {0} or address1 ILIKE {0} or city ILIKE {0} or ID ILIKE {0} and custom.enable = true';
        ...
        var queryString2 = 'city = {0} or city ILIKE {0} and custom.enable = true';
        storeMgrResult = StoreMgr.searchStoresByCoordinates(location.lat, location.long, distanceUnit, 40000, queryString2, city);
        searchKey = { actionUrl: actionUrl };
    } else {
        // find by coordinates (detect location)
        var queryString3;
        if (session.custom.countryCode != null) {
            queryString3= 'custom.enable = true and countryCode = {0}';
        } else {
            queryString3= 'custom.enable = true';
        }
        
        storeMgrResult = StoreMgr.searchStoresByCoordinates(location.lat, location.long, distanceUnit, resolvedRadius, queryString3, session.custom.countryCode);
