...
server.get('Show', consentTracking.consent, server.middleware.https, csrfProtection.generateToken, function (req, res, next) {
    ...
    var productArray = [];
    if(wishlistModel && wishlistModel.items && wishlistModel.items.length >= 1) {
        wishlistModel.items.forEach(element => {
            var ProductMgr = require('dw/catalog/ProductMgr');
            var ProductFactory = require('*/cartridge/scripts/factories/product');
            var productObj = ProductMgr.getProduct(element.pid);
            var productTileParams = { pview: 'tile', 'pid': element.pid};
            ...
            var listObj = {
                ...
                item_id: productObj.hasOwnProperty("masterProduct") ? productObj.masterProduct.ID : productObj.ID,
                item_sku_id: productObj.hasOwnProperty("masterProduct") ? productObj.ID : '',
...
/**
 * * To remove a product from wishlist it is required to call
 * *this with POST
 */
server.post('RemoveProductWishList', function (req, res, next) {
    var list = productListHelper.removeItem(req.currentCustomer.raw, req.querystring.pid, { req: req, type: 10 });
    var listIsEmpty = list.prodList.items.empty;
